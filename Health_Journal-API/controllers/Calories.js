...
module.exports.getCaloriesAverage = async(req,res,next)=>{
    const calorieAverage = await getAverageCalories(req.userId);
    res.status(200).json({
...
module.exports.getHistoryCalories = async(req,res,next)=>{
    const historyCalories = await HistoryCalorie.find({person: req.userId}).select({meet: 1,calories: 1,calories_range: 1,date: 1}).sort({date: 'desc'});
    res.status(200).json({
